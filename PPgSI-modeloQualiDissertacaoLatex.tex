...
\usepackage{graphicx, subfigure}			% Inclusão de gráficos
...
Com maior formalidade, dada uma matriz $X(N,M)$ em que $N$ é o número de linhas, $M$ o número de colunas e $x(m,n)$ é, geralmente, um número real representando a relação entre a linha $x_n$ e a coluna $y_m$, o problema de \textbf{coagruapmento} consiste em encontrar um conjunto $\cal{C}$ de submatrizes $G(I,J)$, onde $I=\{i_1, ..., i_r\}$ com $r \leq L$ e $J=\{j_1, ..., j_s\}$ com $s \leq C$, que maximize a similaridade entre os elementos $g\{i,j\}$.  
...
\subsection{Coclustering by Block Value Decomposition}

Um dos primeiros algoritmos propostos na literatura para a resolução do problema de \textit{coclustering} que se baseia em NMF, é o Decomposição de Valores em Blocos (\textit{Block Value Decomposition} - BVD). Este algoritmo tem o objetivo de clusterizar a matriz de dados $X$ simultâneamente em $k$ clusters de $\mathcal{X}$ exclusivos e $l$ clusters de $\mathcal{Y}$.
...
\begin{problem}[O algoritmo de Coclustering por Decomposição de Valores em Blocos propõem uma solução para o seguinte problema de otimização]
\label{def:bvd1}
	\begin{equation}
		\begin{aligned}
		\underset{U, S, V}{\text{minimizar }} \mathcal{F} = & \underset{U, S, V}{\text{minimizar}}
		& & \norm{X - USV^T}^{2}_{F} \\
		& \text{sujeito a}
...
onde $U \in \mathbb{R}^{n \times k}_{+}$, $S \in \mathbb{R}^{k \times l}_{+}$, $V \in \mathbb{R}^{m \times l}$ e $\norm{\cdot}$ denota a norma de Frobenius para matrizes.

A aproximação $X \approx USV^T$ é capaz de gerar diversas informações que podem ser interpretadas. Analisando as matrizes após a fatoração, a matriz $U$ pode ser interpretada como a indicação de quais $k$ clusters cada objeto $x_{i \cdot}$ $\forall i = \{1, \dots, n\}$ pertence, da mesma forma, a matriz $V$ pode ser interpretada como a indicação de quais dos $l$ clusters cada objeto $y_{\cdot j}$ $\forall j = \{1, \dots, m\}$ pertence. A matriz $S$ também permite interpretação quando é considerado os clusters formados por $U$ e $V$, cada valor $s_{uv}$ $\forall u = \{1, \dots, k\}, \forall v = \{1, \dots, l\}$ representa a intensidade em que o cluster de objetos $u$ e o cluster de características $v$ colaboraram para as suas formações, ou seja, fixando um cluster de objetos $u$, $s_{uv}$ representa a intensidade em que um cluster de características $v$ colaborou para a sua formação, alternadamente, fixando um cluster de características $v$, $s_{uv}$ representa a intensidade em que um cluster de objetos $u$ colaborou para a sua formação.

Seguindo a teoria de otimização não linear com restrições, é possível otimizar a função $\mathcal{F}$ pelo tradicional método de multiplicadores de Lagrange utilizando as condições de KKT. Então, os multiplicadores $\Lambda_1 \in \mathbb{R}^{k \times n}$, $\Lambda_2 \in \mathbb{R}^{l \times k}$ e $\Lambda_3 \in \mathbb{R}^{m \times l}$ são introduzidos.

\begin{problem}[O problema dual lagrangiano da definição~\ref{def:bvd1} é dado pela otimização do seguinte problema]
\label{def:bvd2}
	\begin{equation}
		\begin{aligned}
		\underset{U, S, V, \Lambda_1, \Lambda_2, \Lambda_3}{\text{minimizar}} \mathcal{L} = & \underset{U, S, V, \Lambda_1, \Lambda_2, \Lambda_3}{\text{minimizar}}
		& & \norm{X - USV^T}^{2}_{F} - tr(\Lambda_1 U^T) - tr(\Lambda_2 S^T) - tr(\Lambda_3 V) \\
		\end{aligned}
    \end{equation}
\end{definition}

Para que o problema apresentado da definição~\ref{def:bvd2}, é necessário que as condições de KKT sejam satisfeitas.

\begin{theorem}[Se $U^*$, $S^*$ E $V^*$ são soluções possíveis elas devem satisfazer as seguintes condições, equações xx-xx fixando $S$ e $V$, xx-xx fixando $U$ e $V$, e xx-xx fixando $U$ e $S$]
\label{def:bvd3}
    \begin{equation}
    \label{eq:kkt1}
        \begin{aligned}
            \frac{\partial \mathcal{L}}{\partial U} = 0 & & \text{stationarity} \\
            \Lambda_1 \odot U = 0 & & \text{complementary slackness} \\
            \Lambda_1, U \geq 0 & & \text{feasability}
        \end{aligned}
    \end{equation}
    ...
    \label{eq:kkt2}
        \begin{aligned}
            \frac{\partial \mathcal{L}}{\partial S} = 0 & & \text{stationarity} \\
            \Lambda_2 \odot S = 0 & & \text{complementary slackness} \\
            \Lambda_2, S \geq 0 & & \text{feasability}
        \end{aligned}
    \end{equation}
    ...
    \label{eq:kkt3}
        \begin{aligned}
            \frac{\partial \mathcal{L}}{\partial V} = 0 & & \text{stationarity} \\
            \Lambda_3 \odot V = 0 & & \text{complementary slackness} \\
            \Lambda_3, V \geq 0 & & \text{feasability}
        \end{aligned}
    \end{equation}
\end{theorem}
...
Fazendo uma análise simplista do algoritmo~\ref{algo} é possível perceber que este tem $t$ etapas de otimização, sendo que cada x``Este algoritmo tem complexidade $\mathcal{O}(t(k + l)nm)$.
...
$$\mathcal{L} = |X - USV^T||^2 - tr(\lambda_1U^T) - tr(\lambda_2S^T) - tr(\lambda_3V^T)$$

Then $\mathcal{L}$ must satisfy the K.K.T. conditions:
...
Solving the derivatives and equal them to $0$, is possible to solve the optimization problem by applying gradient ascending on $\mathcal{L}$ with the following update rules:
...
$$\mathcal{F} = \textit{min } ||X - U\sum_{i=1}^{k}I^{(i)}SV^{(i)}||^{2}_{F}$$
% $$\textit{s.t. } U^TU = I$$
...
$$\mathcal{L} = \mathcal{F} - tr(\Lambda_1 U^T) - tr(\Lambda_2 S^T) - \sum_{i=1}^{k} tr(A_i V^{(i)^T})$$
