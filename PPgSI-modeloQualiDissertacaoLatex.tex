...
\label{algo:bvd}
    \begin{algorithmic}[1]
        \Function{BVD}{$X$, $maxIter$}
            ...
            \While{(não convergiu \textcolor{green}{(tem que comentar no texto alguma condição de convergência)}) ou ($t \leq maxIter$)} %\Comment{We have the answer if r is 0}
                \State
                    \begin{equation}
...
O problema de Fatoração tripla rápida de Matrizes Não-negativas (\textit{Fast Non-negative Matrix Tri Factorization} - FNMTF) foi proposto por \citeonline{Wang2011}, com os seguintes argumentos contra o uso prático dos problemas até agora propostos para encontrar cogrupos: os problemas de fatoração em três matrizes propostos até o momento, envolviam soluções algorítmicas iterativas, com multiplicações de matrizes intensas em cada etapa; e pelo fato dos algoritmos propostos realizarem coagrupamento relaxado, necessitando de pós-processamento \textcolor{green}{(como é feito o pós-processamento? deve-se falar disso.)} para a extração dos grupos, o que implica em inúmeras soluções para o agrupamento.
...
\label{eq:ovnmtf:gradV}
    \begin{array}{lcl}
        [\nabla_{V_{(p)}} \mathcal{F}_4] & = & 2 \big( -X^T U \sum_{p'=1}^{k} I_{(p')} S + V_{(p)} S^T I_{(p)} U^T U I_{(p)} S + \sum_{p' \neq p}  V_{(p')} S^T I_{(p')} U^T U I_{(p)} S \big)
    \end{array}
\end{equation}
                ...
                \For{$p \leftarrow 1, k$}
                    \State
                        ...
                        \label{eq:ovnmtf:updateV}
                            V^{(t+1)}_{(p)} \gets V^{(t)}_{(p)} \odot \frac{ \sum_{p'=1}^{k} X^T U^{(t+1) I_{(p)} S^{(t)} } }{ V^{(t)}_{(p)} S^{(t)^T} I_{(p)} U^{(t+1)^T} U^{(t+1)} I_{(p)} S^{(t)} + \sum_{p' \neq p} V^{(t)}_{(p')} S^{(t)^T} I_{(p')} U^{(t+1)^T} U^{(t+1)} I_{(p)} S^{(t)} }
                        \end{equation}
                \EndFor
                \State
                    \begin{equation}
                    \label{eq:ovnmtf:updateS}
                        S^{(t+1)} \gets S^{(t)} \odot \sum_{p=1}^{l} \frac{ I_{(p)} U^{(t+1)^T} X V^{(t+1)^T}_{(p)} }{ \sum_{p'=1}^{l} I_{p} U^{(t+1)^T} U^{(t+1)} I_{(p')} S^{(t)} V^{(t+1)^T}_{(p')} V^{(t+1)}_{(p)} }
                    \end{equation}
                \State $t \gets t + 1$
            \EndWhile\label{euclidendwhile}
...
Esses experimentos foram projetados e executados com o fim de ilustrar as capacidades e limitações dos algoritmos de coclusterição baseados em fatoração de matrizes presentes na literatura e dos algoritmos propostos neste trabalho, todos já apresentados nos capítulos~\ref{ch:fatoracao} e~\ref{ch:proposedalgs}, respectivamente. Tais capacidades e limitações são discutidas neste capítulo em termos de resultados obtidos sobre ambientes controlados (bases de dados sintéticas), ambientes semi-controlados (bases de dados textuais simplificadas) e ambientes aqui denominados não controlados (bases de dados textuais originais). O intuito com a experimentação desses algoritmos em bases de dados textuais é ilustrar seu desempenho como um método de resolução da tarefas agrupamento de textos, da área de mineração de textos. Também, como forma de melhorar a compreensão sobre o desempenho dos algoritmos de coagrupamento, dois algoritmos clássicos de agrupamento foram aplicados sobre as mesmas bases de dados: \textit{k-means} e \textit{fuzzy k-means}.
...
Para proporcionar uma visão organizada das capacidades e limitações dos algoritmos, primeiramente são apresentados os experimentos e os resultados obtidos com as bases de dados sintéticas. Tais resultados são apresentados em termos de qualidade de \textit{reconstrução}, a qual é analisada com apoio de visualizacão gráfica, de qualidade de \textit{particionamento}, a qual é avaliada em termos de medidas de qualidade de grupos de linhas e qualidade de grupos de colunas, \textcolor{red}{e de qualidade de \textit{coagrupamento}, a qual é avaliada via medida xxxxx}. Então, a qualidade de coagrupamento é avalidada nos resultados obtidos com as bases de dados textuais reais originais e simplificadas. Para essas últimas, uma análise qualitativa é delineada de forma a ilustrar o valor agregado que a flexibilidade de modelos de coagrupamento podem trazer ao contexto de mineração de textos.
...
%Em seguida, foram gerados os cogrupos considerando um conjunto de valores centrais $\mathcal{C} = \{ 20, 40, 60, 80, 100, 120, 140, 160, 180 \}$, e escolhendo um valor central $c \in \mathcal{C}$ de forma aleatória, adicionado valores de $0.0$ à $10.0$, gerados a partir da função $unif(0.0, 10.0)$, que gera valores aleatórios uniformes de $0.0$ à $10.0$, para então formar um bicluster.

%Conversar com Lucas, porque essa formulação não está funcionando. Creio que consigo arrumar mas não estou segura de que o meu entendimento acima (no texto) está correto.

\textcolor{cyan}{Assim, considerando que um cogrupo é uma submatriz $B$ da matriz original $X$, ele pode ser gerado pela equação} % ~\ref{eq:genbic}.
...
\textcolor{cyan}{sendo $i$ e $j$ os índices das linhas e colunas de $B$ respectivamente, $b_{ij}$ um elemento de $B$, e $unif(0, 10)$ uma função geradora de números \textcolor{red}{aleatórios} no intervalor $[0,10$], que respeita a distribuição uniforme. E, considerando a matriz original como $X$, um cogrupo é inserido em $X$ fazendo $x_{ij} = b_{ij}, \forall i, j$.}

% ************************************************************
% ************************************************************

Embora a visualização gráfica da figura~\ref{fig:bic-syntetic-structure} seja intuitiva, alguns detalhes em relação à sua interpretação precisam ser cuidadosamente observados. A depender do objetivo da análise de coagruamento, na figura~\ref{fig:bic-syntetic-structure:a}, por exemplo, mais de um cogrupo pode ser observado. Portanto, faz-se útil observar em mais detalhes quais são os cogrupos que podem ser considerados, \textcolor{red}{além do cogrupo que é de interesse neste trabalho}, em cada uma das bases de dados. Para isso, por conveniência de interpretação, considere que um cogrupo é sempre formado por um agrupamento de linhas e um agrupamento de colunas. Assim, tem-se os seguintes cogrupos na base de dados sintética (a):

%Os seguintes cogrupos podem ser encontrados nas matrizes da figura~\ref{fig:bic-syntetic-structure}: considerando que cada matriz têm dimensão $150 \times 150$, então, os índices das linhas estão entre $0$ e $149$ e os índices das colunas também de $0$ à $149$:

\begin{itemize}
\item O mais evidente, destacado em azul, é formado pelas linhas [60 .. 109] e pelas colunas [70 .. 139]. Esse é o cogrupo de interesse, nesse projeto, para a resolução da tarefa de coagrupamento aplicada a dados textuais.
\item O segundo, que não está destacado na visualização, é formados por todas as linhas e as colunas [0 .. 69] e [140 .. 149].
...
\begin{itemize}
\item (b): textcolor{red}{três cogrupos de principal interesse e seis cogrupos não destacados na figura;}
...
% sugiro que seja colocada a descrição detalhada de cada um apenas de tivermos espaço, se o texto não ficar muito longo. Eventualmente, também podemos deixar em Apêndice, ou fazer uma figura também, para o Apêndice.
...
Para execução dos experimentos com o \textit{k-means} os seguintes parâmetros foram estabelecidos: número máximo de iterações ($300$) ou a diferença do erro de minimização \textcolor{red}{(alcançar pelo menos $1e-4$)}, o que ocorrer primeiro, como critérios de parada; número de grupos $k = 2$ para a base de dados sintética (a) e $k = 3$ para as bases de dados sintéticas (b), (c), (d) e (e). \textcolor{red}{A escolha de $k$ foi basicamente realizada com a prerrogativa de que o algoritmo \textit{k-means} deveria encontrar os grupos considerando todos os atributos descritivos, e desta forma, agrupar os dados de acordo com a similaridade total. A escolha de $k$ maiores levariam o algoritmo a, necessariamente, dividir em grupos diferentes os dados que deveriam pertencer a um mesmo grupo.} Foram executadas $10$ rodadas do algoritmo, com \textcolor{red}{inicialização de centróides aleatória}, sendo que o melhor modelo resultante nessas rodadas foi escolhido para ilustrar a avaliação da qualidade da resconstrução.
...
As reconstruções obtidas a partir dos centróides encontrados pelo algoritmo \textit{k-means} são ilustradas Figura~\ref{fig:reconstruction:kmeans}. As matrizes originais são repetidas na figura, nas cinco primeiras posições, de forma a facilitar a análise visual dos resultados. \textcolor{red}{Para um melhor entendimento da representação gráfica, note que uma linha recebe cores de acordo com sua pertinência a um grupo representado por um centróide, ou seja, se a linha $10$ pertencer ao centróide $1$, então os valores da linha $10$ serão os valores do centróide $1$.} \textcolor{cyan}{O problema aqui é que a linha então deveria estar toda em azul. Precisamos explicar a figura direito, pois a ideia não esta batendo. Ou a figura está errada.}
...
% não sei, aqui deveria ser cogrupos de linhas ... tem algo errado nessa interpretação.
Note que o modelo resultante da execução do \textit{k-means} é capaz de realizar a \textcolor{red}{reconstrução perfeita} para as bases de dados (a) à (d). No entanto, o modelo não tem informações sobre a estrutura de cogrupos de \textcolor{cyan}{colunas} presentes nos dados e assim não é capaz de reconstruir o problema apresentado pela base de dados (e). \textcolor{red}{Esse resultado já era esperado, uma vez que para obter tal informação, o algoritmo deveria ser capaz de descobrir similaridades parciais entre os dados, e em sua concepção, o algoritmo não é preparado para isso.}
% O caso (e) é o único que o algoritmo não é capaz de realizar a reconstrução, isso ocorre pois o algoritmo não é capaz de encontrar intersecção de clusters. % ele até é capaz de ver interseção mas não deste tipo, correto?
...
% tanto aqui quanto no kmeans, as cores são diferentes. Então é preciso dar mais detalhes sobre isso para que o leitor não entenda que é um erro.

% verificar se podemos chamar de parâmetro de fuzzyficação
Os mesmos critérios de parada e número de grupos usados para o \textit{k-means} foram também usados nos experimentos com o \textit{fuzzy k-means}. O valor para o parâmetro de fuzzificação $m$ foi mantido em $2$. Também, $10$ execuções foram realizadas, com iniciação aleatória de pesos e o melhor modelo obtido foi usado para avaliação da qualidade de resconstrução obtida.
% Há uma justificativa para m=2, vamos colocar aqui.
% TODO eu li que esse é o usual (default), explicar pq

Da mesma forma que o experimento anterior, as reconstruções apresentadas na figura~\ref{fig:reconstruction:fkmeans}, foram obtidos por meio da coloração de linhas de acordo com os centróides obtidos, e as matrizes originais foram repetidas para facilitar a análise visual. % aqui tem fuzzificação, verificar com Lucas.
...
\centering
    ...
    \caption{
        \textcolor{red}{As primeiras cinco matrizes são as matrizes originais, as demais são suas respectivas reconstruções, realizadas a partir dos resultados obtidos com o algorimto \textit{fuzzy k-means}}.
    }
    \label{fig:reconstruction:fkmeans}
\end{figure}

De forma semelhante ao experimento de reconstrução com o \textit{k-means}, aqui também o algoritmo é capaz de realizar a \textcolor{red}{reconstrução} perfeita dos casos (a) à (d). Porém, a reconstrução para o caso (e) não foi obtida com sucesso. Embora o \textit{fuzzy c-means} lide, em sua concepção, com sobreposição de grupos, neste caso representada pelas pertinências fuzzy dos dados a vários grupos, a similaridade parcial em termos de atributos também não é tratada. \textcolor{cyan}{Entender melhor com o Lucas e ver como explicar isso aqui ... .. ... }
...
Para execução dos experimentos com o FNMTF a seguinte parametrização foi estabelecida: número máximo de iterações ($1000$) ou o erro de minimização (\textcolor{red}{alcançar pelo menos $1e-4$)}, o que ocorrer primeiro, como critérios de parada; o número de cogrupos de linhas \textcolor{red}{($k$)} e colunas \textcolor{red}{($l$)} configurados da seguinte maneira: $k = l = 2$ para (a) e $k = l = 3$ para (b), (c), (d) e (e). Novamente, escolhas baseadas no conhecimento \textit{aprioi} que se tem sobre a quantidade de cogrupos, e quais cogrupos, deseja-se obter. Para a execução do algoritmo \textit{ONMTF} é necessária a normalização dos dados, então, todas as matrizes de dados sintéticas foram normalizadas para que a norma das linhas fosse igual à $1$. % náo precisaríamos indicar a operação aqui?

A partir da realização de $10$ execuções do algoritmo, foi escolhido o modelo que alcançou a menor taxa de erro e a partir do resultado obtido a reconstrução foi avaliada. A qualidade das resconstruções pode ser visualmente observada na figura~\ref{fig:reconstruction:onmtf}. A resconstrução foi obtida a partir da multiplicação das matrizes fatoradas, ou seja, \textcolor{red}{$U, S$ e $V^T$}, conforme explicado no capítulo~\ref{ch:fatoracao}.
...
Analisando as reconstruções realizadas, é possível perceber que a reconstrução é realizada com êxito nos casos (a), (b) e (c). \textcolor{red}{O algoritmo falhou em reconstruir a matriz nas colunas se interseccionam no caso (d) e nas linhas que se intersecionam no caso (e). A falha da reconstrução é percebida na coloração diferenciada, formando regiões com sombras, nas colunas, ou linhas, envolvidas nos cogrupos que possuem alguma interseção.}
% estava o contrário, ver com Lucas

\textcolor{red}{A coloração diferenciada daquela esperada (seguindo a matriz original) é resultante do estado da matriz $V$, que contém as relações de associação de linhas e/ou colunas aos cogrupos. Para o caso (d), por exemplo, é possível deduzir, a partir da análise visual, que há valores de magnitude diferentes estabelecendo a associação das colunas X x X, Y x Y e Z x Z, ao grupo de linhas X x X. Isso é possível pois o algoritmo não força a associação única e exclusiva entre uma linha e um cocluster ou uma coluna e um cocluster.Em uma análise qualitativa, poder-se-ia então dizer que o algoritmo está indicando a formação de quatro grupos de colunas em relação ao mesmo grupo de linhas X x X.}
...
%Para esse experimento, foi feita a implementação do algoritmo \textit{FNMTF} usando a linguagem Python.

% porque o onmtf foi tão longo e esse foi tão curto quanto o kmeans e o fcm?????
Para execução dos experimentos com o FNMTF a seguinte parametrização foi estabelecida: número máximo de iterações ($300$) ou o erro de minimização (\textcolor{red}{alcançar pelo menos $1e-4$)}, o que ocorrer primeiro, como critérios de parada; o número de cogrupos de linhas \textcolor{red}{($k$)} e colunas \textcolor{red}{($l$)} configurados da seguinte maneira: $k = l = 2$ para (a) e $k = l = 3$ para (b), (c), (d) e (e). Para a execução do algoritmo ONMTF é necessária a normalização dos dados, então, todas as matrizes de dados sintéticas foram normalizadas para que a norma das linhas fosse igual à 1.

A partir da realização de $10$ execuções do algoritmo, foi escolhido o modelo que alcançou a menor taxa de erro e a partir do resultado obtido a reconstrução foi avaliada. A qualidade das reconstruções pode ser visualmente observada na figura~\ref{fig:reconstruction:fnmtf}. A reconstrução foi obtida a partir da multiplicação das matrizes fatoradas, ou seja, $F, S$ e $G^T$, conforme explicado no capítulo~\ref{ch:fatoracao}.   %$$F*S*G^T$
...
Este caso é semelhante ao anterior (algoritmo \textit{ONMTF}). O algoritmo conseguiu realizar a reconstrução com êxito dos casos (a), (b) e (c), falhando na reconstrução dos casos (d) e (e), onde há intersecção de colunas ou linhas nos cogrupos de interesse. No entanto, nesse caso o algoritmo restringe a associação de linhas, ou colunas, a apenas um cogrupo, e por isso a informação referente a intersecção é totalmente descaracterizada. Observe, por exemplo, que a reconstrução no caso (d) se assemelha à reconstrução do caso (c), embora as matrizes originais nos dois casos sejam diferentes, e representam informação qualitativa sobre associação de dados e atributos em cogrupo também diferenciada.

\subsection{Reconstrução a partir dos resultados do algoritmo \textit{OvNMTF}}
\label{subsec:results-reconstruction-ovnmtf}

% Para esse experimento, foi feita a implementação do algoritmo \textit{OvNMTF} usando a linguagem Python.

% por que l = 2????? não deveria ser l = 3?????

Para execução dos experimentos com o \textit{OvNMTF} a seguinte parametrização foi estabelecida: número máximo de iterações ($1000$) ou o erro de minimização (\textcolor{red}{alcançar pelo menos $1e-4$)}, o que ocorrer primeiro, como critérios de parada; o número de cogrupos de linhas \textcolor{red}{($k$)} e colunas \textcolor{red}{($l$)} configurados da seguinte maneira: $k = l = 2$ para (a) e $k = 3$ e \textcolor{red}{$l = 2$} para (b), (c), (d) e (e). Novamente, escolhas baseadas no conhecimento \textit{aprioi} que se tem sobre a quantidade de cogrupos, e quais cogrupos, deseja-se obter. Para a execução do algoritmo \textit{OvNMTF} é necessária a normalização dos dados, então, todas as matrizes de dados sintéticas foram normalizadas para que a norma das linhas fosse igual à $1$. % náo precisaríamos indicar a operação aqui?
...
O algoritmo conseguiu realizar a reconstrução com êxito dos casos (a), (b), (c) e (d), com valores suavemente diferentes (isso pode ser notado pela cor das matrizes reconstruídas), devido à normalização necessária. Note que o algoritmo \textit{OvNMTF} é capaz de lidar com cogrupos com intersecção de colunas, e portanto é capaz de resolver o caso (d). Como o algoritmo não é capaz de lidar com cogrupos com intersecção nas linhas, ele não é capaz de realizar a reconstrução do caso (e), e, assim como o experimento com \textit{ONMTF}, o \textit{OvNMTF}, esse têm a informação de associação de cada linha ou coluna com cada cogrupo e por isso, preserva a informação de intersecção, porém de maneira fragmentada em vários cogrupos.
% Por isso, as intersecções são preservadas, observando as áreas das intersecções de linhas com sombra, na matriz de dados sintéticos.
...
%Para esse experimento, foi feita a implementação do algoritmo \textit{BinOvNMTF} usando a linguagem Python.
...
% TODO: citação
% precisa colocar o indice explicitamente. Acredito que possamos colocá-lo no capítulo 2.
O índice usado para a avaliação de particionamento foi o \textit{Rand Index}. Esse índice gera valores entre $0$ e $1$, com $0$ indicando que o particionamento de dados gerado pelo modelo não concorda em nenhum par de pontos com o particionamento real e $1$ indicando que o particionamento gerado pelo modelo é exatamente igual ao particionamento real.

% eu não entendi porque dessa observação aqui: verificar com Lucas. % voltar a observação mas entrar outra na parte da reconstrução
...
A avaliação externa foi realizada para o particionamento de linhas e para o particionamento de colunas e os resultados estão listados na tabela~\ref{tab:datasetsstats}. Observando os valores destacados na coluna \textit{Rand Index} (linhas), é possível notar que nenhum dos algoritmos foi capaz de particionar perfeitamente (com valor $1.0$) as linhas para a base de dados (e), que contém intersecção de linhas. Claramente, isso ocorre pois nenhum dos algoritmos é capaz de encontrar cogrupos com intersecção de linhas.
...
Na coluna \textit{Rand index} (colunas) da tabela~\ref{tab:datasetsstats}, também é possível observar que não há resultados para os algoritmos \textit{k-means} e \textit{fuzzy k-means}, pois os mesmos não resolvem o problema de particionamento de colunas. Os valores destacados desta mesma coluna mostram que os algoritmos propostos neste trabalho (\textit{OvNMTF} e \textit{BinOvNMTF}) são capazes de particionar as colunas corretamente tanto para o caso (d) quanto para o caso (e).
...
\textcolor{cyan}{É possível correlacionar os resultados obtidos nas reconstruções com os resultados obtidos através da validação com o \textit{Rand Index}. Então tem que colocar algo mais direto .....}
...
\subsection{Experimentos qualitativos}
