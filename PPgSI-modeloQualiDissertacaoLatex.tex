...
As reconstruções presentes nas subfiguras da Figura~\ref{fig:reconstruction:ovnmtf}, foram construídas a partir da multiplicação das matrizes fatoradas, ou seja, $F \sum_{p=1}^{k} I_{(p)} S G_{(p)}^T$.
...
As reconstruções presentes nas subfiguras da Figura~\ref{fig:reconstruction:binovnmtf}, foram construídas a partir da multiplicação das matrizes fatoradas, ou seja, $F S G^T$.
...
Os resultados são independentes da organização das linhas das matrizes, ou seja, com ou sem embaralhamento é obtido os mesmos resultados.
