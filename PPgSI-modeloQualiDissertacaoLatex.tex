            ...
            \While{(não convergiu) ou ($t \leq maxIter$)} %\Comment{We have the answer if r is 0}
                \State
                    ...
                    \State
                        ...
                        \begin{equation}
                        \label{eq:ovnmtf:updateV}
                            \textcolor{green}{ V^{(t+1)}_{(p)} \gets V^{(t)}_{(p)} \odot \frac{ X^T U^{(t+1)} I_{(p)} S^{(t)} }{ \sum_{p'=1}^{k} V^{(t)}_{(p')} S^{(t)^T} I_{(p')} U^{(t+1)^T} U^{(t+1)} I_{(p)} S^{(t)} }}
                        \end{equation}
                ...
                \State
                    ...
                    \begin{equation}
                    \label{eq:ovnmtf:updateS}
                        \textcolor{green}{ S^{(t+1)} \gets S^{(t)} \odot  \frac{ \sum_{p=1}^{l} I_{(p)} U^{(t+1)^T} X V^{(t+1)}_{(p)} }{ \sum_{p=1}^{l} \sum_{p'=1}^{l} I_{p} U^{(t+1)^T} U^{(t+1)} I_{(p')} S^{(t)} V^{(t+1)^T}_{(p')} V^{(t+1)}_{(p)} }}
                    \end{equation}                \State $t \gets t + 1$
            \EndWhile\label{euclidendwhile}
