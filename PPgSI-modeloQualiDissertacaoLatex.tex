...
% chamei de uni-dimensional porque o algoritmo poderia resolver sobreposição de colunas, ou de linhas, a depender de como o problema é visto. Então, colocar sobreposição de colunas eu achei que ficaria incompleto e colocar só sopreposição eu achei que ficar mal especificado.
...
% Lucas, eu pensei em colocar aqui uma figura que tenha os quadradinhos com sobreposição de colunas, e também um outro com sobreposição de linhas e um exemplo contextualizado notícias. 
...
% se ficar a ideia de unidimensional, talvez isso fosse interessante aqui no nome do problema.
...
% pergunta, se entraram essas regras também não tinha que entrar aqui a regra que permitir reescrever o problema (expandir o problema)???
...
Expandindo $\mathcal{F}_4$, \textcolor{red}{com base nas propriedade de traço de matrizes}, para tornar o cálculo do gradiente mais simples, é possível obter: %a seguinte equação:

% pessoal, não deveria ser F4 linha???
...
%Para o cálculo do gradiente de $\mathcal{F}_4'$ a estratégia usada é a apresentada em \citeonline{Yoo2010}:
...
% não precisaria específicar o que é o + e o - ????? Isso é auto explicativo?
...
Finalmente,  para o cálculo de $\nabla_V_(p) \mathcal{F}_4$, considere as partes positiva e negativa, $[\nabla_V_(p) \mathcal{F}_4]^+$ e $[\nabla_V_(p) \mathcal{F}_4]^-$, respectivamente. Usando a igualdade da equação~\ref{eq:diff:AQB}, com $A = X^T U I_{(p)} S$, $Q = V^{T}_{(p)}$, e $B = I$, $\forall p$, é possível obter $[\nabla_{V_{(p)}} \mathcal{F}_4]^-$, da seguinte forma:
\[
    \begin{array}{lcl}
...
% condições de convergência não foram provadas, mas podemos dar algum insight sobre isso?

% Eu tirei essa nota pois não há nada mais sobre isso no cap 3 correto. Há a necessidade de chamar a atenção para isso mesmo? Se sim, é preciso apenas referenciar o artigo.
...
\begin{equation}
    \begin{array}{lclc}
        \displaystyle \mathcal{F}_5(F, S, G_{(1)}, \dots, G_{(k)}) & = & \displaystyle \min_{F, S, G_{(1)}, \dots, G_{(k)}} & \norm{ X - F \sum_{p=1}^{k} I_{(p)} S G_{(p)}^T }^{2}_{F} \\
                                                                   ...
                                                                   &   &                              & \textcolor{blue}{S \geq 0}, \\ % não tem isso no problema 3
                                                                   &   &                              & G_{(p)} \in \Psi^{m \times l}, \quad \forall p
    \end{array}   \nonumber
\end{equation}
\end{problem}

% o X não está definido nos outros problemas. Até estava em um mas eu retirei pois estava somente em um deles. Ou coloca em todos ou não coloca em nenhum

% nos outros algoritmos também não tem o + na definicão das matrizes no texto, só como restrição no problema.
em que  $S \in \mathbb{R}^{k \times l}_{+}$, $\Psi = \{0, 1\}$, \textcolor{red}{$\sum_{p=1}^{k} F_{\cdot p} = 1$ e $\sum_{q=1}^{l} G_{\cdot q}_p = 1$}, $p = \{1, \dots, k\}$ é o índice para o conjunto de matrizes $\mathcal{G} = \{ G_{(1)}, \dots, G_{(k)} \}$, $I_{(p)} \textcolor{red}{\in \{0,1\}^{k \times k}}$ é uma matriz identidade seletora, \textcolor{red}{$k \ll n$, $l \ll m$} e $\norm{\cdot}_F$ denota a norma de Frobenius. \textcolor{red}{Na formulação, o papel das matrizes $I_k$ seletoras é, justamente, organizar a base de grupos de linhas, de forma que cada um deles seja otimizado de acordo com uma das matrizes $G_k$.}

% aqui não tem derivação????
...
% eu passei a chamar de estratégia ao invés de algoritmo - precisamos padronizar isso
